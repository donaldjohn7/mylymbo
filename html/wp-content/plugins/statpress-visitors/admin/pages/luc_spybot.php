	...
	{ // Bot Spy
		if ($robot <> $rk->spider)
		{
			...
			echo "<IMG style='border:0px;height:16px;align:left;' alt='" . $title . "' title='" . $title . "' SRC='" . STATPRESS_V_PLUGIN_URL . '/images/spider/' . $img . "'>
									...
									<tr><td valign='top' width='170'><div><font size='1' color='#3B3B3B'><strong>" . luc_hdate($rk->date) . " " . $rk->time . "</strong></font>
									</div></td>
									<div><td>".((isset($rk->post_title)) ? $rk->post_title : luc_post_title_Decode(urldecode($rk->urlrequested)))."</td></div></td></tr>";
			...
			if ($num_row < $LIMIT_PROOF)
			{
				echo "<tr>
						...
						<td><div>".((isset($rk->post_title)) ? $rk->post_title : luc_post_title_Decode(urldecode($rk->urlrequested)))."</div></td></tr>";
				$num_row += 1;
			}
		echo "</div></td></tr>\n";
	}
